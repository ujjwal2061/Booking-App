...
import { BiLoaderAlt } from 'react-icons/bi';
 ...
 export default  function PlaceFrom(){
    ...
    return (
        ...
        <form onSubmit={hanleformsubmit} className=" relative bg-white rounded-md p-6 md:p-8 ">
          ...
          <textarea value={extraInfo}  onChange={(e) => setExtraInfo(e.target.value)} placeholder="Additional information guests should know"
          className="w-full px-4 py-2 rounded-lg border focus:ring-2  outline-none mb-6 transition-all"/>
          ...
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-3 mt-2 mb-6">
           ...
           <div>
            ...
            <div>
              ...
              <input type="text" value={maxguests} onChange={(e) => setMaxguests(e.target.value)} placeholder="4"
                className="w-full px-4 py-2 rounded-lg border focus:ring-2  outline-none transition-all" />
            </div>
          </div>
          ...
          {error&&<p className='absolute  top-8 right-2 bg-red-700 rounded-md  px-6 py-3 font-mono text-white'>{error}</p>}
        </form>
      </div>
    )
}